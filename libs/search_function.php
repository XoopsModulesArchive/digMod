...
// $xoopsDB->conn set in connect.php file
...
if (($limite != 10) && ($limite != 30) && ($limite != 100)) { $limite = 10; }
...
else { $my_path = $path; $path = stripslashes($path); }
...
if (PHPDIG_LOGS == true && !$browse && !$refine) {
   ...
   if (is_array($final_result)) {
       phpdigAddLog ($xoopsDB->conn,$option,$strings,$exclude,count($final_result),$search_time);
   }
   else {
       phpdigAddLog ($xoopsDB->conn,$option,$strings,$exclude,0,$search_time);
   }
}
$timer->stop('Logs');
...
$powered_by_link = "<font size=\"1\" face=\"verdana,arial,sans-serif\"><a href=\"http://www.phpdig.net/\">Powered by PhpDig.</a></font>";
...
$html="";
$html.=phpdigMakeForm($query_string,$option,$limite,SEARCH_PAGE,$site,$path);
$html.="<h3><span class=\"phpdigMsg\">$result_message</span>
<br /><span class=\"phpdigAlert\">$ignore_message</span>
<br /><span class=\"phpdigAlert\">$ignore_commess</span>
</h3>
</div>";
if (is_array($table_results)) {
       while (list($n,$t_result) = each($table_results)) {
             $html.="<p \n";
             $html.="<b>$n. [".$t_result['weight']." %]&nbsp;&nbsp;".$t_result['page_link']."</b>\n<br />\n";
             $html.="<blockquote>\n";
             $html.=$t_result['text'];
             $html.="</blockquote>\n";
       }
